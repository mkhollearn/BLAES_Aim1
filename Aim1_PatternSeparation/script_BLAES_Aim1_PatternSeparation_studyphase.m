function param = script_BLAES_Aim1_PatternSeparation_studyphase(settings,ALL_IMAGES,slash,trials,breakoutnum_iter,practice_pairs)
...
for idx_iter = 6501:6500+size(practice_pairs)[0];
    if practice_pairs{idx,4} ~= "Foil"
        ...
        param.Stimuli.Value{9,idx_iter}      = practice_pairs{idx,4}; %novelty condition
    end
    idx = idx + 1;
end 
...
i = 1;
for idx_iter = 6501:6500+size(practice_pairs)[0];
%   if ~strcmp(practice_pairs{idx,4},'Foil')
 %       continue
  %  else
