function param = script_BLAES_Aim1_PatternSeparation_testphase(settings,ALL_IMAGES,slash,trials,breakoutnum_iter, practice_pairs)
...
% 5200:      ISI
% 5301-6319: Confidence Ratings
% 6501-6510: Practice images
% 6601-6610: Practice confidence images



% Set up Stimuli Parameteres
...
param.Stimuli.RowLabels{8}  = 'Subsequent Stim/NoStim';
param.Stimuli.RowLabels{9}  = 'NoveltyCondition';
param.Stimuli.RowLabels{10} = 'ConfidenceRating';
...
param.Stimuli.Value{10,idx}     = '';

%% Practice images 6501
idx = 1;
for idx_iter = 6501:6500+size(practice_pairs)[0];
     if ~strcmp(practice_pairs{idx,4},'Targ')
        param.Stimuli.ColumnLabels{idx_iter} = sprintf('%d',idx_iter);
        param.Stimuli.Value{1,idx_iter}      = '';
        param.Stimuli.Value{2,idx_iter}      = sprintf('%s',fullfile(settings.BCI2000path, 'tasks', 'BLAES_Aim1', 'Aim1_PatternSeparation','practice_images',practice_pairs{idx,2}));
        param.Stimuli.Value{3,idx_iter}      = '';
        param.Stimuli.Value{4,idx_iter}      = settings.TestDuration;
        param.Stimuli.Value{5,idx_iter}      = '0';      
        param.Stimuli.Value{6,idx_iter}      = 'image'; 
        param.Stimuli.Value{7,idx_iter}      = 'KeyDown == 37 || KeyDown == 39'; %left and right arrows
        param.Stimuli.Value{8,idx_iter}      = '0'; % No stim for practice 
        param.Stimuli.Value{9,idx_iter}      = practice_pairs{idx,4};
        param.Stimuli.Value{10,idx_iter}     = ''; 

        idx = idx + 1;
     else 
        param.Stimuli.ColumnLabels{idx_iter} = sprintf('%d',idx_iter);
        param.Stimuli.Value{1,idx_iter}      = '';
        param.Stimuli.Value{2,idx_iter}      = sprintf('%s',fullfile(settings.BCI2000path,'tasks', 'BLAES_Aim1', 'Aim1_PatternSeparation','practice_images',practice_pairs{idx,1}));
        param.Stimuli.Value{3,idx_iter}      = '';
        param.Stimuli.Value{4,idx_iter}      = settings.TestDuration;
        param.Stimuli.Value{5,idx_iter}      = '0';      
        param.Stimuli.Value{6,idx_iter}      = 'image'; 
...
%KeyDown == 67 C || KeyDown == 86 V || KeyDown == 66 B || KeyDown == 78 N

%% Test images 1-2314
for idx = 1:length(ALL_IMAGES)
    param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
    param.Stimuli.Value{1,idx}      = '';
    param.Stimuli.Value{2,idx}      = sprintf('%s',fullfile(settings.BCI2000path, 'tasks', 'BLAES_Aim1', 'Aim1_PatternSeparation','PS_images_fixed',ALL_IMAGES(idx).name));
    param.Stimuli.Value{3,idx}      = '';
    param.Stimuli.Value{4,idx}      = settings.TestDuration;
    param.Stimuli.Value{5,idx}      = '0';      
    param.Stimuli.Value{6,idx}      = 'image'; 
    param.Stimuli.Value{7,idx}      = 'KeyDown == 37 || KeyDown == 39'; % left and right arrows
    param.Stimuli.Value{8,idx}      = ''; 
    param.Stimuli.Value{9,idx}      = '';
    param.Stimuli.Value{10,idx}     = '';
end 
...
for idx = 1:length(trials)
    ...
    param.Stimuli.Value{9,trials{idx,4}} = trials{idx,3}; %novelty
end

%% inter-trial interval ITI (fixation cross) 2801
for idx = 2801:5115
    ...
    param.Stimuli.Value{7,idx}      = 'KeyDown == 37 || KeyDown == 39'; %left and right arrows for 2 response, where 37 = old and 39 = new
    param.Stimuli.Value{8,idx}      = ''; 
    param.Stimuli.Value{9,idx}      = ''; 
    param.Stimuli.Value{10,idx}     = '';
end 
...
%% Confidence Ratings 5301 - 6319
conf_images = dir(fullfile(settings.BCI2000path, 'tasks', 'BLAES_Aim1', 'Aim1_PatternSeparation', 'PS_images_RO','*.PNG'));
c = 1;
for indx = 5301:5300+length(conf_images)
     param.Stimuli.ColumnLabels{indx} = sprintf('%d',indx);
     param.Stimuli.Value{1,indx}      = '';
     param.Stimuli.Value{2,indx}      = sprintf('%s', fullfile(settings.confidencepath(c).folder,settings.confidencepath(c).name)); %need absolute path
     param.Stimuli.Value{3,indx}      = '';
     param.Stimuli.Value{4,indx}      = settings.InstructionDuration;
     param.Stimuli.Value{5,indx}      = '0';    
     param.Stimuli.Value{6,indx}      = 'confidence_rating'; 
     param.Stimuli.Value{7,indx}      = 'KeyDown == 37 || KeyDown == 39'; %left and right arrows for 2 response, where 37 = sure and 39 = not sure
     param.Stimuli.Value{8,indx}      = ''; 
     param.Stimuli.Value{9,indx}      = '';
     param.Stimuli.Value{10,indx}     = ''; 

     c = c+1;
end 

%% Practice confidence answers

prac_conf_images = dir(fullfile(settings.BCI2000path, 'tasks', 'BLAES_Aim1', 'Aim1_PatternSeparation', 'practice_conf_RO','*.PNG'));

count = 1;
for indx = 6601:6600+length(prac_conf_images)
     param.Stimuli.ColumnLabels{indx} = sprintf('%d',indx);
     param.Stimuli.Value{1,indx}      = '';
     param.Stimuli.Value{2,indx}      = sprintf('%s', fullfile(settings.prac_confpath(count).folder,settings.prac_confpath(count).name)); %need absolute path
     param.Stimuli.Value{3,indx}      = '';
     param.Stimuli.Value{4,indx}      = settings.InstructionDuration;
     param.Stimuli.Value{5,indx}      = '0';    
     param.Stimuli.Value{6,indx}      = 'practice_confidence_rating'; 
     param.Stimuli.Value{7,indx}      = 'KeyDown == 37 || KeyDown == 39'; %left and right arrows for 2 response, where 37 = sure and 39 = not sure
     param.Stimuli.Value{8,indx}      = ''; 
     param.Stimuli.Value{9,indx}      = '';
     param.Stimuli.Value{10,indx}     = ''; 
...
%% Sequence
taskseq = [5200]; %ISI index
for i = 1:length(trials) %trials is 120 images (one breakout session)
    taskseq = [taskseq trials{i,4} 5300+trials{i,4} 5200 2800+trials{i,4}]; %trials{i,4} is the index of the image
end            %image, confidence, ISI, and ITI

seq = [2601 2503 taskseq 2504 2601]'; %sequence of the entire study from start to finish
     %syncpulse, test instructions, task seq, end instructions, syncpulse

% Practice sequence
prac_seq = [5200];
i = 1;
for idx_iter = 6501:6500+size(practice_pairs)
    prac_seq = [prac_seq idx_iter 6600+(i) 5200];
            %ISI, prac image, confidence, ISI
    i = i + 1;
end

prac_seq = [2601 2503 2502 prac_seq 2601]'; 
           %sync pulse, instruction, instruction2, image sequence, sync
           %pulse
...
save(['test_param_value', num2str(breakoutnum_iter),'.mat'], 'param_value')
save(['test_task_sequence', num2str(breakoutnum_iter), '.mat'], 'taskseq')

%% write the param struct to a bci2000 parameter file
parameter_lines = convert_bciprm( param );
fid = fopen(strcat(settings.parm_filename, '_test_breakout_', num2str(breakoutnum_iter), '.prm'), 'w');
...
%% Write test practice parameter file
param.Sequence.Value        = cellfun(@num2str, num2cell(prac_seq), 'un',0);
param.Sequence.NumericValue = prac_seq;
parameter_lines = convert_bciprm( param );
fid = fopen(strcat(settings.parm_filename, '_test_practice_breakout_', num2str(breakoutnum_iter), '.prm'), 'w');
